...
/*****Speed based compensation of filter is no longer required and hence commented*******
	...
	else if (s->OmegaFltred < Q15(OMEGA6))
	{
		s->ThetaOffset = FracMpy(s->OmegaFltred,SLOPEFRAC6);
		s->ThetaOffset += s->OmegaFltred * SLOPEINT6;
		s->ThetaOffset += CONSTANT6;
	}
	else if (s->OmegaFltred < Q15(OMEGA7))
	{
		s->ThetaOffset = FracMpy(s->OmegaFltred,SLOPEFRAC7);
		s->ThetaOffset += s->OmegaFltred * SLOPEINT7;
		s->ThetaOffset += CONSTANT7;
	}
	else if (s->OmegaFltred < Q15(OMEGA8))
	{
		s->ThetaOffset = FracMpy(s->OmegaFltred,SLOPEFRAC8);
		s->ThetaOffset += s->OmegaFltred * SLOPEINT8;
		s->ThetaOffset += CONSTANT8;
	}
	else if (s->OmegaFltred < Q15(OMEGA9))
	{
		s->ThetaOffset = FracMpy(s->OmegaFltred,SLOPEFRAC9);
		s->ThetaOffset += s->OmegaFltred * SLOPEINT9;
		s->ThetaOffset += CONSTANT9;
	}
	else if (s->OmegaFltred < Q15(OMEGA10))
	{
		s->ThetaOffset = FracMpy(s->OmegaFltred,SLOPEFRAC10);
		s->ThetaOffset += s->OmegaFltred * SLOPEINT10;
		s->ThetaOffset += CONSTANT10;
	}
	else
